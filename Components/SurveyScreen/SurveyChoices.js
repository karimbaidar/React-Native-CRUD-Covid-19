...
export default class SurveyChoices extends Component {
  ...
  InsertIntoStamm = () => {
    fetch(
      'http://YOUR_PUBLIC_IP_ADDRESS/com/baidar/Components/Syncing/SurveyQuestionaire.php',
      {
        method: 'POST',
        headers: {
    ...
    if (this.state.flagInsertion != 1) {
      ...
      db.addProduct_history(data)
        .then(result => {
          ...
          // Alert.alert("insertion to History Successful");
        })
        .catch(err => {
          console.log(err);
          this.setState({
